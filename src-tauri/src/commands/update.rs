...
const GITHUB_API: &str =
    "https://api.github.com/repos/Jacksonary/super-kafka/releases/latest";
const GITEE_API: &str =
    "https://gitee.com/api/v5/repos/weiguoliu/super-kafka/releases/latest";
...
async fn fetch_github() -> Result<(String, String), String> {
    ...
    let url = resp
        .html_url
        .unwrap_or_else(|| "https://github.com/Jacksonary/super-kafka/releases".to_string());
    Ok((tag, url))
}
...
async fn fetch_gitee() -> Result<(String, String), String> {
    ...
    let tag = resp.tag_name.ok_or("no tag_name")?;
    let url = format!(
        "https://gitee.com/weiguoliu/super-kafka/releases/tag/{}",
        tag
    );
    Ok((tag, url))
}
